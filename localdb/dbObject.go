 ...
 */
type SDBar struct {
	Code               string    // 股票的代码，上证股票以sh开头，深证股票以sz开头
	Date               time.Time // 交易日期
	Open               float64   // 开盘价
	High               float64   // 最高价
	Low                float64   // 最低价
	Close              float64   // 收盘价
	Change             float64   // 涨跌幅，复权之后的真实涨跌幅，保证准确。
	Volume             float64   // 成交量
	Money              float64   // 成交额
	Trade_market_value float64   // 流通市值
	Market_value       float64   // 总市值
	Turnover           float64   // 换手率
	Adjust_price       float64   // 后复权价，复权开始时间为股票上市日，精确到小数点后10位。
	Report_type        string    // 最近一期财务报告的类型，3-31对应一季度，6-30对应半年报，9-30对应三季度，12-31对应年报。
	Report_date        time.Time // 最近一期财务报告实际发布的日期。
	PE_TTM             float64   // 最近12个月市盈率，股价/最近12个月归属母公司的每股收益TTM
	PS_TTM             float64   // 最近12个月市销率，股价/最近12个月每股营业收入。
	PC_TTM             float64   // 最近12个月市现率，股价/最近12个月每股经营现金流。
	PB                 float64   // 市净率，股价/最近期财报每股净资产
	Adjust_price_f     float64   // 前复权价，复权开始时间为股票最近一个交易日，精确到小数点后10位
}
...
func (this SDBar) GetSDDBar() SDDBar {
	return SDDBar{
		...
		this.Volume,             // 成交量
		this.Money,              // 成交额
		this.Trade_market_value, // 【流通市值】 流通市值，流通市值 / 股价 = 流通股股本
...
func (this SDBar) GetTrade_market_value() float64 {
	return this.Trade_market_value
}
...
type SDDBar struct {
	Code               string    // 股票的代码，上证股票以sh开头，深证股票以sz开头
	Date               time.Time // 【交易日期】 本周市场的最后一个交易日
	Open               float64   // 开盘价
	High               float64   // 最高价
	Low                float64   // 最低价
	Close              float64   // 收盘价
	...
	Volume             float64   // 成交量
	Money              float64   // 成交额
	Trade_market_value float64   // 【流通市值】 流通市值，流通市值 / 股价 = 流通股股本
	Market_value       float64   // 【总市值】 总市值，总市值 / 股价 = 总股本
	Turnover           float64   // 【换手率】 成交量 / 流通股本  = 成交量*股价/流通市值
	Adjust_price       float64   // 后复权价，复权开始时间为股票上市日，精确到小数点后10位。
	Adjust_price_f     float64   // 前复权价，复权开始时间为股票最近一个交易日，精确到小数点后10位
	Trade_days         int       // 【交易天数】 本周股票实际交易的天数
	Period             string    // 【K线周期】可能为 周K线，月K线，季K线，其值分别 W M Q
}
...
func (this *SDDBar) GetDBName() string {
	...
	switch this.Period {
	case "W":
		dbName = DBConfig.DBName.StockMarketW1
	case "M":
		dbName = DBConfig.DBName.StockMarketM1
	case "Q":
		dbName = DBConfig.DBName.StockMarketQ1
	default:
		dbName = DBConfig.DBName.StockMarketW1
	}
	return dbName
...
// 【流通市值】 流通市值，流通市值 / 股价 = 流通股股本
func (this *SDDBar) GetTrade_Market_value() float64 {
	return this.Trade_market_value
}
...
type IDBar struct {
	Index_code string    // 指数的代码，上证股票以sh开头，深证股票以sz开头
	Date       time.Time // 交易日期
	Open       float64   // 开盘价
	Close      float64   // 收盘价
	Low        float64   // 最低价
...
func (this *IDBar) GetDBName() string {
	...
	return DBConfig.DBName.IndexMarketRawD1
}
...
func parseToDBObject(line []byte) (DBObject, error) {
	var array []string
	// 去除换行符
	strLine := string(line)
	...
	if fieldCount == 20 {
		...
		sdd.Trade_market_value, _ = strconv.ParseFloat(array[9], 64)
